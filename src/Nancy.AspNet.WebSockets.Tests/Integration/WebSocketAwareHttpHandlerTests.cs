...
namespace Nancy.AspNet.WebSockets.Tests.Integration
{
    public class The_WebSocketAwareHttpHandler_class
    {
        ...
        {
            public With_site_not_overriding_the_default_nancy_handler()
                : base("StandardSite")
            {
            }
            ...
            {
                var text = Http.GetBodyAsString("http://" + GetHost());
            ...
            public async void Should_notify_a_handler_when_the_client_closes_the_socket()
            {
                ...
                // Get the log
                var newlineSeparated = Http.GetBodyAsString("http://" + GetHost() + "/log");
